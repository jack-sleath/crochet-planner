...
export function ColourPalette({ palette, onPaletteChange }: Props) {
  ...
  function handleHexChange(value: string) {
    ...
    if (parsed) { setPicked(parsed); setHexValid(true) }
    else setHexValid(false)
  }

  function addColour() {
    if (hexValid && !palette.includes(picked)) onPaletteChange([...palette, picked])
  }
    ...
    <section className="palette-section" aria-label="Colour palette">
      <div className="palette-header">
        ...
        <button
          ...
          aria-label={collapsed ? 'Expand colour palette' : 'Collapse colour palette'}
        >
          {collapsed ? '▸' : '▾'}
        </button>
      </div>
